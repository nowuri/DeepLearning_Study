...
imagenet_valid_data = keras.preprocessing.image_dataset_from_directory('/home/ivpl-d28/dataset/Imagenet/imagenet/val')
...
# 데이터 전처리 적용
augmented_train_data = imagenet_train_data.map(lambda x, y: (data_augmentation(x), y))
augmented_valid_data = imagenet_valid_data.map(lambda x, y: (data_augmentation(x), y))
augmented_test_data = imagenet_test_data.map(lambda x, y: (data_augmentation(x), y))
...
x1 = Dropout(0.7)(x1)
x1 = Dense(units=1000, activation='softmax',kernel_initializer='he_normal')(x1)
...
x2 = Dropout(0.7)(x2)
x2 = Dense(units=1000, activation='softmax',kernel_initializer='he_normal')(x2)
...
x = Flatten()(x)
# softmax
out = Dense(units=1000, activation='softmax',kernel_initializer='he_normal')(x)
...
# train model
history= model.fit(augmented_train_data, batch_size=128, epochs=50, verbose=1,
...
# Accuracy 그리기
plt.plot(history.history['dense_5_sparse_categorical_accuracy'], label='Training Accuracy')
